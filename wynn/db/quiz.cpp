...
int Quiz::questionLevel() const {
    ...
    if ( questionIndex() >= questionCount() ) {
        QLOGX("Index exceeded question count!");
        return DEFAULT_VAL;
    }
...
{
    static const QString DEFAULT_VAL{};
    if ( questionIndex() >= questionCount() ) {
        QLOGX("Index exceeded question count!");
        return DEFAULT_VAL;
    }
...
void Quiz::answerCorrect() {
    Q_ASSERT(stats_.curQuestion < questionCount());
    stats_.correct++;
    questions_[stats_.curQuestion++].setResult(SUCCESS);
}

void Quiz::answerIncorrect() {
    Q_ASSERT(stats_.curQuestion < questionCount());
    stats_.incorrect++;
    questions_[stats_.curQuestion++].setResult(FAIL);
}

void Quiz::answerUnsure() {
    Q_ASSERT(stats_.curQuestion < questionCount());
    stats_.unsure++;
    questions_[stats_.curQuestion++].setResult(NOCHANGE);
}
